function textStims = makeCatLocTextImages(displayName, imgDir)

%Load in parameters: 
params = catLoc_Params;
nPos = params.stimLocs.n;

%Do you want to save PNGs of the images? 
showImages = false;
if showImages
...
stringIs = NaN(nCat, nPos, nImg);
fovealI = find(params.stimLocs.eccs == 0);
periphIs = find(params.stimLocs.eccs ~= 0);
...
for ci=1:nCat
    ...
    switch params.textCategoryWordTypes{ci}
        ...
        case {'lowFreqWords', 'lowFreqWordsFovea'}
            %choose 
    ...
    %assign foveal strings, perhaps according to freq
    stringIs(ci, fovealI, :) = is(theseIs);
    
    unusedIs = setdiff(is, stringIs(ci, fovealI, :));
    
    %assign strings all the other positions, avoiding string repeats at any
    %one position, and as few repeats overall as possible
    %(unless we're in teh one condition with only 1 word per image, at
    %fovea)
    if ~strcmp(allCats{ci}, 'lowFreqWordsFovea')
        for ii=1:nImg
            for pp = 1:nPosPeriph
                posI = periphIs(pp);
                ...
                if ~isempty(unusedIs)
                    ...
                    %indices not yet used at this position, or in this image
                    thisPosNotUsedIs = setdiff(is, union(stringIs(ci, :, ii), stringIs(ci, posI, :)));

                    if ~isempty(thisPosNotUsedIs)
                        thisI = thisPosNotUsedIs(randsample(1:length(thisPosNotUsedIs), 1));

                    else
                        fprintf(1,'\nWarning: no unused strings to use\n');
                        keyboard
                    end
                end

                stringIs(ci, posI, ii) = thisI;
...
for ci=1:nCat
    for pp=1:nPos
        ...
        nRepeats = nImg - length(unique(squeeze(stringIs(ci, pp, :))));
        if nRepeats>0
            fprintf(1,'\n(%s)WARNING - not enough %s at position %i (ecc=%.1fdeg): %i repeats out of %i\n', mfilename, params.textCategories{ci}, pp,  params.stimLocs.eccs(pp), nRepeats, nImg);
            pause(0.5);
...
for pcf = 1:length(params.pseudofontCategories)
    switch params.pseudofontCategories{pcf}
        case 'PseudoCourier'
            ...
            %matchCats =  {'highFreqWordsCourier','lowFreqWordsCourier','pseudowordsCourier','consonantsCourier'};
            thisFont ='BACS2';
        case 'PseudoSloan'
            matchCatIs = find(strcmp(params.textCategoryFonts, 'Sloan'));
            %matchCats =  {'highFreqWordsSloan','lowFreqWordsSloan','lowFreqWordsFoveaSloan','pseudowordsSloan','consonantsSloan'};
            thisFont = 'PseudoSloan';

    end
    %matchCatIs = find(ismember(params.textCategories, matchCats));
    for mci=matchCatIs
...
for ci = 1:nCat
    ...
    if strcmp(allCats{ci}, 'lowFreqWordsFoveaSloan')
        posToDo = fovealI;
    else
        posToDo = 1:nPos;
    end
    ...
    for ii = 1:nImg
        for fi=fontsToDo
            ...
            for pi=posToDo
                stringIndex = stringIs(ci,pi,ii);
                
                thisXHeight = t.xHeightsDeg(pi);
                sizeI = find(t.unqXHeightsDeg==thisXHeight);
                
                %create the 'alphabet' and pull out glyph images
                ...
                if startX==0
                    startX=1;
                    endX = endX+1;
                    fprintf(1,'\n\nMoved a word right 1 pixel\n')
                end
                if endX==(size(img,2)+1)
                    endX = endX - 1;
                    startX = startX-1;
                    fprintf(1,'\n\nMoved a word left 1 pixel\n')
                end
            ...
            if showImages
                ...
                if ii<=params.org.totalImgPerCategory
                    ...
                    %name, so that doesnt need to be added in again
                        imgFile = fullfile(pngDir, sprintf('%s_%i.png', allCats{ci},   ii));
                    end
                else
                    imgFile = fullfile(pngDir, sprintf('%s_%i_PRAC.png', allCats{ci}, ii-params.org.totalImgPerCategory));
                end
                saveas(gcf,imgFile);
            ...
            if ii<=params.org.totalImgPerCategory 
                if length(fontsToDo)>1
                    imgFile = fullfile(imgDir, sprintf('%s_%s_%i.mat', allCats{ci}, t.fontNames{fi}, ii));
                else
                    %Change in 2022: the category name icludes the font
                    %name, so that doesnt need to be added in again
                 imgFile = fullfile(imgDir, sprintf('%s_%i.mat', allCats{ci}, ii));
                end
            else %save the images over totalImgPerCategory as practice images 
                imgFile = fullfile(imgDir, sprintf('%s_%i_PRAC.mat', allCats{ci}, ii-params.org.totalImgPerCategory));
            end
...
textStims.meanWordsExtentDeg = squeeze(mean(wordsExtentPx,2))/pixPerDeg;
textStims.minWordsExtentDeg = squeeze(min(wordsExtentPx,[],2))/pixPerDeg;
textStims.maxWordsExtentDeg = squeeze(max(wordsExtentPx,[],2))/pixPerDeg;
