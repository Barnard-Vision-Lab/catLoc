function task = catLoc_SetRunAndTrialOrder_Unilateral(task)
%this version puts 1 image per trial

%% Find stimulus images
task.imagePath =  fullfile(task.projPath, 'stimuli_unilateral', task.displayName);
...
if ~isempty(task.objectCategories) 
    limbParamFile = fullfile(task.imagePath,sprintf('objectStimParams_%s.mat',task.displayName));
    if ~exist(limbParamFile,'file')
        error('(%s) No object images made for this display (%s)!',mfilename, limbParamFile);
    end
    %load face image params
    load(limbParamFile,'objectStims');
...
if ~isfield(task, 'allRunTrials')
    ...
    %set categories on each trial within each run separately 
    for ri=1:task.numRuns %loop through runs, create a little table "rt" for each one then append rt to the bigger table T
        rt = table;
        ...
        %append this run's table (rt) to the whole experiment table, T
        T = [T; rt];
    end

    %% here you could set the "side" for each trial, for each category separately, but taking into account trials across all runs in the "T" table
    for cati = 1:task.nCategories
        %sides = [1 1 2 3 3];
        catTrls = find(T.categoryI==cati);
        ncatTrls = length(catTrls);
        nTrialsBySide = round(ncatTrls*task.stimLocs.propTrialsBySide);
        sidesToSample = [];
        for si=1:task.stimLocs.n
            sidesToSample = [sidesToSample si*ones(1, nTrialsBySide(si))];
        end
        %sampledsides = randsample(sides, ncatTrls, true);
        if length(sidesToSample)~=ncatTrls
            fprintf(1,'\n(%s) Failed to properly control stimulus side for correct num trials\n', mfilename);
            keyboard
        end
        T.side(catTrls) = sidesToSample(randperm(ncatTrls)); %randomly shuffle
    end
    ...
    for ci = nonBlankCategories
        ...
        if ci<=length(task.textCategories)
            ...
            allCentralStimIs = squeeze(textStims.stringIs(strcmp(task.categories{ci}, textStims.valsByIndex.category), :));
            ...
            switch task.categories{ci}
                ...
                case {'faces','maleFaces','femaleFaces'}
                    ...
                    %take out the stimulus indices for the foveal stimulus in
                    %all images we have for this condition
                    allCentralStimIs = squeeze(faceStims.faceIs(strcmp(task.categories{ci}, faceStims.valsByIndex.category), :));
                    doCheckRepeat = true;
                case {'objects', 'objects1','objects2'}
                    ...
                    %take out the stimulus indices for the foveal stimulus in
                    %all images we have for this condition
                    allCentralStimIs = squeeze(objectStims.objectIs(strcmp(task.categories{ci}, objectStims.valsByIndex.category), :));
                    doCheckRepeat = true;
                case {'limbs', 'limbs1','limbs2'}
                    ...
                    %take out the stimulus indices for the foveal stimulus in
                    %all images we have for this condition
                    allCentralStimIs = squeeze(limbStims.limbIs(strcmp(task.categories{ci}, limbStims.valsByIndex.category), :));
                    doCheckRepeat = true;
        ...
        if doCheckRepeat && ~task.practice
            try
            ...
            centralStim(repIs) = 9999; %dont include stimulus that actually ought to repeat
        ...
        for trli = 1:N
            ...
            for sii=1:task.org.stimPerTrial
                ...
                thisFileName = sprintf('%s_%i_side%i', stimFileCategories{trli}, imgNums(trli, sii), T.side(trlN));
